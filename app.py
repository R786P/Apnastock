...
# ✅ FIXED: Admin route now serves templates/admin.html
@app.route('/admin')
def admin():
...
# ✅ NEW: POST /api/products - Add new product
@app.route('/api/products', methods=['POST'])
def add_product():
    """Add new product - accepts JSON with base64 images"""
    try:
        data = request.get_json()
        if not data:
        ...
        if not name or not price or not affiliate_link:
            return jsonify({'error': 'Name, price and affiliate link required'}), 400
        ...
        if conn:
            ...
            if product:
                return jsonify({'message': 'Product added successfully!', 'id': product[0]}), 201
    except Exception as e:
...
# ✅ FIXED: PUT now accepts JSON (not form data)
@app.route('/api/products/<product_id>', methods=['PUT'])
def update_product(product_id):
    """Update existing product - accepts JSON"""
    try:
        data = request.get_json()
        if not data:
            return jsonify({'error': 'No data provided'}), 400
        ...
        if conn:
            ...
            if product:
                return jsonify({'message': 'Product updated successfully!', 'id': product[0]}), 200
            ...
            if conn:
                ...
                cur.execute('''
                    ...
                    (order_id, customer_name, customer_email, customer_phone,
                     product_id, product_name, product_price, quantity, total_price,
                     status, created_at)
...
def get_user_orders():
    ...
    try:
        ...
        if conn:
            ...
            cur.execute('SELECT * FROM orders WHERE customer_email = %s ORDER BY created_at DESC', (user_email,))
            rows = cur.fetchall()
            cur.close()
            conn.close()
